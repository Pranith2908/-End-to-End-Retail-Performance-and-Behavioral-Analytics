create database retail_business_questions;

USE retail_business_questions;
GO
...
-- SALES: Add surrogate key instead of composite key
...
-- RETURNS references SALES using sales_id

-- First add sales_id column to returns
ALTER TABLE returns_cleaned
ADD sales_id INT;
...
REFERENCES sales_cleaned(sales_id);
...
ALTER TABLE sales_cleaned
ADD discount_pct_new DECIMAL(6,4),
    total_amount_new DECIMAL(12,2);

UPDATE sales_cleaned
SET discount_pct_new =
CAST(DATEDIFF(SECOND, '00:00:00', discount_pct) AS FLOAT) / 86400;
